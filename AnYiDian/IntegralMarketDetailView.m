...
#import "IntegralMarketDetailView.h"
#import "Community.h"
...
@interface IntegralMarketDetailView ()<UIWebViewDelegate, MWPhotoBrowserDelegate>
{
    UserInfo *userInfo;
...
@implementation IntegralMarketDetailView
    ...
    [[AFOSCClient sharedClient] getPath:findRegUserInfoUrl parameters:nil
                                success:^(AFHTTPRequestOperation *operation, id responseObject) {
                                    ...
                                    else
                                    {
                                        ...
                                        [self.buyNowBtn setTitle:[NSString stringWithFormat:@"积分兑换(当前积分:%@)", integral] forState:UIControlStateNormal];
                                    }
...
- (void)initDetailView
{
    ...
    self.integralLb.text = [NSString stringWithFormat:@"%d", commodityDetail.integral];
...
- (IBAction)buyNowAction:(id)sender {
    if([integral integerValue] < commodityDetail.integral)
    {
    ...
    item.integral = commodityDetail.integral;
    item.number = 1;
    item.ischeck = @"1";
    ...
    IntegralConfirmationView *confirmationView = [[IntegralConfirmationView alloc] init];
    confirmationView.integral = integral;
